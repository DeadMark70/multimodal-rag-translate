...
_BENCHMARK_METHOD_EXCLUSION_KEYWORDS = (
    "supervision",
    "cross-entropy",
    "pseudo-label",
    "loss",
...
def _looks_like_benchmark_intent(lowered_question: str) -> bool:
    """Detect benchmark intent while avoiding methodology-only false positives."""
    has_metric_keyword = any(keyword in lowered_question for keyword in _BENCHMARK_METRIC_KEYWORDS)
    has_context_keyword = any(keyword in lowered_question for keyword in _BENCHMARK_CONTEXT_KEYWORDS)
    has_methodology_exclusion = any(
        keyword in lowered_question for keyword in _BENCHMARK_METHOD_EXCLUSION_KEYWORDS
    )
    has_hard_signal = any(
        keyword in lowered_question
        for keyword in ("benchmark", "baseline", "flops", "params", "計算效率", "比較基準")
    )

    if has_hard_signal and (has_metric_keyword or has_context_keyword):
        return True
    if has_metric_keyword and has_context_keyword and not has_methodology_exclusion:
        return True
    return False
...
def classify_question_intent(question: str) -> QuestionIntent:
    ...
    if any(keyword in lowered for keyword in _FIGURE_FLOW_KEYWORDS):
        return "figure_flow"

    if _looks_like_benchmark_intent(lowered):
