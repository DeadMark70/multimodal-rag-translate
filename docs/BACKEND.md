...
- GraphRAG now has two contract surfaces:
  ...
  - Any auto, locator, or graph-to-chunk flag combination source-expands or skips; it never falls back to a raw graph prompt. Raw graph context is permitted only for the explicit `graph_raw_current` compatibility mode with no structured graph flag enabled. Planning decisions are recorded as gate events but never bundle, merge, or prompt graph evidence.
