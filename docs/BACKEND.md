...
- GraphRAG now has two contract surfaces:
  ...
  - `router_auto_graph` classifies each question from explicit execution hints: relationship/claim-scope queries may use graph retrieval, exact table/figure/formula/numeric questions use it only as a locator when the asset registry is explicitly available, and non-graph questions leave vector retrieval unchanged. Explicit manual override is locator-only.
  ...
  - Structured graph retrieval reuses evaluation graph-event recording. Events retain the mode's explicit feature-flag/evidence-mode snapshot, route and gate reason, candidate/resolved/final counts in the route reason, and source chunk/document identifiers in evidence lifecycle rows; community hints are not final evidence.
