"""PostgreSQL persistence for evaluation campaigns."""
...
from evaluation.postgres import (
    RepositoryConnection,
    connect_db as connect_postgres,
    force_init_db as force_init_db,
    init_db as init_db,
)
...
@asynccontextmanager
async def connect_db() -> AsyncIterator[RepositoryConnection]:
    """Borrow a PostgreSQL connection for an evaluation repository."""
    async with connect_postgres() as connection:
        yield RepositoryConnection(connection)


async def _table_columns(connection: RepositoryConnection, table_name: str) -> set[str]:
...
def _row_to_campaign_status(row: Mapping[str, Any]) -> CampaignStatus:
...
def _row_to_campaign_result(row: Mapping[str, Any]) -> CampaignResult:
    execution_profile = (
...
def _row_to_agent_trace_detail(row: Mapping[str, Any]) -> AgentTraceDetail:
...
def _row_to_agent_trace_summary(row: Mapping[str, Any]) -> AgentTraceSummary:
