...
def test_claim_verifier_provider_uses_synthesizer_with_strict_schema(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    boundary = importlib.import_module("data_base.agentic_v9.provider_boundary")
    raw_provider = object()
    ...
    schema = boundary.claim_verification_response_schema()
    assert schema["type"] == "object"
    assert "verdicts" in schema["properties"]
    assert boundary.build_claim_verifier_provider(response_schema=schema) is bound_provider
    assert observed["purpose"] == "synthesizer"
    assert observed["provider"] is raw_provider
