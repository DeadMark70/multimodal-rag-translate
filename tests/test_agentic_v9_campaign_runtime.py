...
def test_same_source_and_locator_chunk_supports_only_matching_slot_fact() -> None:
    scope = ResolvedSourceScope(authorized_doc_ids=["ukan"])
    contract = QueryContract(
        contract_version="2",
        route="multi_document_exact",
        intent="two Table 3 metrics",
        required_slots=[
            RequiredSlot(
                slot_id="S5",
    ...
    task = RetrievalTask(
        task_id="Q16:round-1:table-3",
        round_id="round-1",
        query_id="Q16",
        query="U-KAN proposed method Table 3 metrics",
        target_slot_ids=["S5", "S6"],
        source_scope=scope,
        locator_hints=["Table 3"],
    )
    results = (
                ...
                chunks=[
                    {
                        "doc_id": "ukan",
                        "chunk_id": "chunk-ukan",
                        "text": "Table 3 reports the U-KAN metric as 0.8.",
                        "table_id": "Table 3",
                    }
                ],
            ),
    ...
    assert [packet.slot_ids for packet in packets] == [["S5"]]
...
def test_v1_locator_hint_accepts_ordinary_retrieved_chunk_without_metadata() -> None:
    ...
    contract = QueryContract(
        contract_version="1",
        route="exact_structured",
        intent="legacy table lookup",
        required_slots=[
            RequiredSlot(
                slot_id="legacy-slot",
                description="legacy table result",
                locator_hints=["Table 3"],
            )
        ],
        locator_hints=["Table 3"],
    ...
    task = RetrievalTask(
        task_id="legacy:round-1:group",
        round_id="round-1",
        query_id="legacy",
        query="legacy table result",
        target_slot_ids=["legacy-slot"],
        source_scope=scope,
        locator_hints=["Table 3"],
    )
    results = (
                ...
                chunks=[
                    {
                        ...
                        "text": "The ordinary retrieved chunk contains the result.",
                    }
                ],
            ),
        ),
    ...
    assert [packet.slot_ids for packet in packets] == [["legacy-slot"]]
    assert packets[0].locator.section == "retrieved_context"
