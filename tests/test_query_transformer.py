...
@pytest.mark.asyncio
async def test_v9_query_transformer_uses_the_injected_budgeted_invoker():
    class RecordingInvoker:
        def __init__(self) -> None:
            self.calls = []

        async def invoke(self, *, phase, purpose, messages):
            self.calls.append((phase, purpose, messages))
            return MagicMock(content="1. alternate query")

    invoker = RecordingInvoker()
    queries = await QueryTransformer(llm_invoker=invoker).generate_multi_queries(
        "original question"
    )

    assert queries == ["original question", "alternate query"]
    assert invoker.calls[0][0:2] == ("query_rewrite", "query_rewrite")


@pytest.mark.asyncio
async def test_v9_multi_query_helper_threads_the_invoker_to_the_production_transformer():
    class RecordingInvoker:
        async def invoke(self, *, phase, purpose, messages):
            assert (phase, purpose) == ("query_rewrite", "query_rewrite")
            return MagicMock(content="1. alternate query")

    assert await transform_query_multi(
        "original question", llm_invoker=RecordingInvoker()
    ) == ["original question", "alternate query"]
