...
@pytest.mark.asyncio
async def test_v10_pipeline_packs_raw_top1_only() -> None:
    ...
    reranker = MagicMock()
    first = Document(page_content="Architecture evidence", metadata={"doc_id": "doc-1", "page_number": 2})
    second = Document(page_content="Benchmark evidence", metadata={"doc_id": "doc-2", "page_number": 4})
    ...
    assert v10["context_pack"]["strategy"] == "top1_raw"
    assert v10["context_pack"]["top1_evidence_count"] == 2
    assert len(v10["source_document_mapping"]) == 2
...
@pytest.mark.asyncio
async def test_v10_packs_top1_raw_chunk_without_neighbor_lookup() -> None:
    decomposer = MagicMock()
    decomposer.decompose = AsyncMock(return_value=[
        SubQueryItem(id="SQ1", query="query", focus="focus", target_entity="entity")
    ])
    evidence = Document(page_content="Only raw evidence", metadata={"doc_id": "doc-1", "page_number": 1})
    reranker = MagicMock()
...
async def test_v10_does_not_invoke_a_map_model() -> None:
    ...
    evidence = Document(page_content="Raw evidence", metadata={"doc_id": "doc-1", "page_number": 1})
    reranker = MagicMock()
    reranker.rerank_with_scores.return_value = [(evidence, 0.9)]
    service = AgenticV10PipelineService(decomposer=decomposer, reranker=reranker)
    synthesis_llm = MagicMock()
...
def test_v10_export_keeps_raw_trace_behind_existing_export_switches() -> None:
    trace = MagicMock(
        execution_profile="agentic_eval_v10_top1_raw",
        agentic_v10={
            "schema_version": "5",
    ...
    assert summary["summary"]["context_strategy"] == "top1_raw"
    assert summary["summary"]["neighbor_evidence_count"] == 0
    ...
    assert raw["raw_payload"]["context_pack"]["strategy"] == "top1_raw"
