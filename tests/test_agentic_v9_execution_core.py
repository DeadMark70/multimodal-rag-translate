...
@pytest.mark.asyncio
@pytest.mark.parametrize(("repair_cap", "support_round"), [(1, 1), (2, 2)])
async def test_successful_last_allowed_repair_keeps_completion_terminal_reason(
    repair_cap: int, support_round: int
) -> None:
    contract = _contract().model_copy(update={"max_repair_rounds": repair_cap})
    terminal_reasons: list[str] = []
    retrieved_repair_rounds = 0
    ...
    def candidates(
        results: tuple[TaskRetrievalResult, ...], _: QueryContract
    ) -> tuple[EvidencePacket, ...]:
        nonlocal retrieved_repair_rounds
        if results and ":repair-" in results[0].task_id:
            retrieved_repair_rounds += 1
        return (_packet(),) if retrieved_repair_rounds == support_round else ()
    ...
    assert retrieved_repair_rounds == support_round
    assert terminal_reasons == ["evidence_complete"]
