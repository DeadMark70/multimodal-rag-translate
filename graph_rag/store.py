...
class GraphStore:
    ...
    def __init__(self, user_id: str, storage_dir: str | Path | None = None) -> None:
        ...
        self._storage_dir = (
            self._current_snapshot_dir(self._root_storage_dir) or self._root_storage_dir
        )
        ...
        payload = {
            "documents": [
                ...
                for status in sorted(
                    self.document_statuses.values(), key=lambda item: item.doc_id
                )
            ]
        }
    ...
    def _write_asset_links(self) -> None:
        ...
        validated_links = [
            GraphAssetLink.model_validate(link.model_dump(mode="python"))
        ...
        self._atomic_write_json(
            self._get_asset_links_path(),
            {
                "assets": [
                    ...
                    for link in sorted(validated_links, key=lambda item: item.asset_id)
                ]
            },
        )
    ...
    def _load_legacy_metadata(self, legacy_data: Dict[str, Any]) -> None:
        ...
        self.communities = [Community(**c) for c in legacy_data.get("communities", [])]
        ...
        self.graph_dirty = (
            bool(self._pending_count) or self.index_version < GRAPH_INDEX_VERSION
        )
    ...
    def _load_metadata(self, legacy_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        if metadata_path.exists():
            try:
                ...
                self.communities = [Community(**c) for c in data.get("communities", [])]
                ...
                self.node_vector_dirty = bool(
                    data.get("node_vector_dirty", self.graph_dirty)
                )
                ...
                logger.warning(
                    "Failed to load graph metadata for user %s: %s", self.user_id, e
                )
                if legacy_data:
    ...
    def _load_document_statuses(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Failed to load graph document statuses for user %s: %s",
                self.user_id,
                e,
            )
    ...
    def _load_extraction_manifests(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Failed to load graph extraction manifests for user %s: %s",
                self.user_id,
                exc,
            )
            return
        for item in payload.get("runs", []):
            try:
                self.extraction_manifests.append(
                    GraphExtractionRunManifest.model_validate(item)
                )
            except (TypeError, ValueError) as exc:
                logger.warning(
                    "Skipping malformed graph extraction manifest for user %s: %s",
                    self.user_id,
                    exc,
                )
    ...
    def _load_edge_provenance(self) -> None:
        ...
        try:
            ...
            if isinstance(payload, dict) and isinstance(payload.get("edges"), list):
                items = [
                    GraphEdgeProvenance.model_validate(item)
            ...
            self.edge_provenance = {
                provenance.edge_id: provenance for provenance in items
            }
        except (json.JSONDecodeError, OSError, TypeError, ValueError) as e:
            logger.warning(
                "Failed to load graph provenance for user %s: %s", self.user_id, e
            )
    ...
    def _load_asset_links(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Failed to load graph asset links for user %s: %s", self.user_id, exc
            )
    ...
    def _load_alias_indexes(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Failed to load graph alias indexes for user %s: %s", self.user_id, exc
            )
    ...
    def record_edge_provenance(
        self, edge_id: str, anchors: List[EvidenceAnchor]
    ) -> None:
    ...
    def record_asset_link(self, link: GraphAssetLink) -> None:
        ...
        validated = GraphAssetLink.model_validate(link.model_dump(mode="python"))
    ...
    def has_usable_asset_links(
        ...
        return any(
            ...
            and bool(
                link.source_chunk_id or (link.text_or_markdown and link.asset_text_hash)
            )
            for link in self.asset_links.values()
        )
    ...
    def upsert_canonical_entity(
        ...
        *,
        ...
        if resolved_type in AUTO_MERGE_ENTITY_TYPES:
            existing_id = self._find_exact_canonical_name(
                normalized_name, resolved_type
            )
        ...
        review_status: Literal["auto", "needs_review", "reviewed"] = (
            "needs_review" if resolved_type in REVIEW_REQUIRED_ENTITY_TYPES else "auto"
        )
        if existing_id is None:
            if resolved_type in AUTO_MERGE_ENTITY_TYPES:
            ...
            if existing is None:
                ...
                existing = CanonicalEntity(
                    canonical_id=existing_id,
                    canonical_name=existing_node.label
                    if existing_node
                    else canonical_name,
                    entity_type=resolved_type,
                    aliases=[],
    ...
    def remove_document(self, doc_id: str) -> int:
        ...
        for source, target, data in self.graph.edges(data=True):
            ...
            if doc_id in edge_docs:
                ...
                if not edge_docs:
                    relation = data.get("relation", "related")
                    edge_id = data.get("edge_id") or self.edge_id(
    ...
    def get_latest_extraction_manifest(
        self, doc_id: str
    ) -> Optional[GraphExtractionRunManifest]:
        """Return the newest successful extraction manifest for one document."""
        return next(
            (
                manifest
                for manifest in reversed(self.extraction_manifests)
